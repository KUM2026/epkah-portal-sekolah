...
@section('title', 'Maklumat Sekolah')
...
<div class="max-w-7xl mx-auto p-6 space-y-6">
    ...
    {{-- Header Card --}}
    <x-welcome-card
        nama="Profile Sekolah"
        :isAdmin="'simple'"
        tagline="Lihat maklumat penuh sekolah anda di sini."
        :verified="false"
        buttonText="Kembali ke Pengguna"
        :buttonRoute="route('sekolah.dashboard')"
    ...
    <div class="bg-green-50 p-6 rounded-2xl shadow-md border border-green-100 space-y-4">
        ...
        <div class="grid grid-cols-12 gap-4">
            <div class="col-span-12 sm:col-span-6 flex flex-col gap-1">
                <p class="text-gray-500 text-sm flex items-center gap-2"><i class="fas fa-id-card text-green-600 w-4"></i> Nama Pentadbir</p>
                <div class="bg-white px-3 py-2 rounded shadow-inner">{{ $sekolah['admin_name'] ?? '-' }}</div>
            </div>

            <div class="col-span-12 sm:col-span-6 flex flex-col gap-1">
                <p class="text-gray-500 text-sm flex items-center gap-2"><i class="fas fa-envelope text-green-600 w-4"></i> E-mel Pentadbir</p>
                <div class="bg-white px-3 py-2 rounded shadow-inner">{{ $sekolah['admin_email'] ?? '-' }}</div>
            </div>

            <div class="col-span-12 flex flex-col gap-1">
                <p class="text-gray-500 text-sm flex items-center gap-2"><i class="fas fa-building text-green-600 w-4"></i> Pejabat Pendidikan Daerah</p>
                <div class="bg-white px-3 py-2 rounded shadow-inner">{{ $sekolah['education_office'] ?? '-' }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-blue-50 p-6 rounded-2xl shadow-md border border-blue-100 space-y-4">
        ...
        <div class="grid grid-cols-12 gap-4">
            ...
            <div class="col-span-12 sm:col-span-6 flex flex-col gap-1">
                <p class="text-gray-500 text-sm flex items-center gap-2"><i class="{{ $field['icon'] }} text-blue-600 w-4"></i> {{ $field['label'] }}</p>
                <div class="bg-white px-3 py-2 rounded shadow-inner">{{ $field['value'] }}</div>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="bg-yellow-50 p-6 rounded-2xl shadow-md border border-yellow-100 space-y-4">
        ...
        <div class="grid grid-cols-12 gap-4">
            <div class="col-span-12 sm:col-span-6 flex flex-col gap-1">
                <p class="text-gray-500 text-sm flex items-center gap-2"><i class="fas fa-university text-yellow-600 w-4"></i> Nama Bank</p>
                <div class="bg-white px-3 py-2 rounded shadow-inner">{{ $sekolah['bank'] ?? '-' }}</div>
            </div>

            <div class="col-span-12 sm:col-span-6 flex flex-col gap-1">
                <p class="text-gray-500 text-sm flex items-center gap-2"><i class="fas fa-hashtag text-yellow-600 w-4"></i> No. Akaun</p>
                <div class="bg-white px-3 py-2 rounded shadow-inner">{{ $sekolah['no_akaun'] ?? '-' }}</div>
            </div>
        </div>
    </div>
